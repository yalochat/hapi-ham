...
exports.register = (server, options, next) => {
  ...
  server.decorate('reply', 'notify', function (message, _options) {
    let response
    ...
    switch (options.provider) {
      case 'facebook-messenger': {
        ...
        if (userId) {
          ...
          const url = config.proxy ? `${config.proxy}/bots/${_options.botSlug ? _options.botSlug : ''}` : fbUrl + tokenParam
          request.server.log('debug', `URL ${url}`)
          Wreck.post(url, { payload: payload }, (err, res, payload) => {
            if (err) {
              request.server.log('error', `Error ${err}`)
              return reject(err)
            }
            request.server.log('debug', 'Facebook messenger response ->')
            request.server.log('debug', payload.toString())
