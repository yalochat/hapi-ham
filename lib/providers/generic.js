...
const getSend = (request, config, logger) => {
  const sendFuncs = {
    facebook: sendFunctions['facebook'](request, config, logger),
    whatsapp: sendFunctions['whatsapp'](request, config, logger),
    smooch: sendFunctions['smooch'](request, config, logger)
  }
  ...
  return (message, _options) => {
    ...
    const send = sendFuncs[provider] || sendFuncs['facebook']
...
const process = (request, config, logger) => {
  ...
  return {
    event: {},
    send: getSend(request, config, logger),
  }
}
