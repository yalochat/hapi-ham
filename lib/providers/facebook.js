...
const getSend = (data, config, logger, event) => (message, _options) => {
  let tokenParam = ''
  const recipientKey = event && event.optin ? 'user_ref' : 'id'
  const recipientId = event && event.optin && event.user_ref
  ...
  const suffix = config.source === 'customer_chat_plugin' ? '-webchat-plugin' : ''

  const url = config.proxy
    ? `${config.proxy}/${_options.botSlug || ''}${suffix}?${config.extraParams}`
...
const process = (request, config, logger) => {
  ...
  if (event) {
    ...
    config.source = _.get(event, 'tags.source', 'messenger')
