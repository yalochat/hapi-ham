...
const createClient = (options) => {
  ...
  const logger = bucker.createLogger({
    name: 'kafka-client',
    console: this.config.debug,
  })
  const { error } = joi.validate(this.config, schema)

  // Validate incoming options for kafka client
  if (error) {
    logger.info('An error has been ocurred when validating options for kafka-client plugin')
  }
  ...
  producer.on(
    'event.error',
    config.errorListener || defaults.errorListener(logger),
  )
